...
def lgb_modelfit_nocv(params, dtrain, dvalid, predictors, label_train, label_validate, objective='binary', metrics='auc',
    ...
    xgtrain = lgb.Dataset(dtrain[predictors].values, label=label_train.values,
                          feature_name=predictors,
                          categorical_feature=categorical_features
                          )
    xgvalid = lgb.Dataset(dvalid[predictors].values, label=label_validate.values,
                          feature_name=predictors,
                          categorical_feature=categorical_features
                          )
...
def DO(frm,to,fileno):
    ...
    for i, (train_fold, validate) in enumerate(kf):
        ...
        train_df, val_df, label_train, label_validate =  X.iloc[train_fold], X.iloc[validate], train_label.iloc[train_fold], train_label.iloc[validate]
        ...
        best_trees.append(best_iteration)
        cv_pred += bst.predict(test_df[predictors], num_iteration=bst.best_iteration)
        cv_train[validate] += bst.predict(val_df[predictors])

        score=metrics.roc_auc_score(label_validate,cv_train[validate])
        print(score)
    ...
    print("Predicting...")
    sub['is_attributed'] = cv_pred /NFOLDS
    if not debug:
        print("writing...")
...
nrows=184903891-1
nchunk=400000#40000000#40000000
val_size=25000#2500000
