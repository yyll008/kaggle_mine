...
def DO(frm,to,fileno):
    ...
    #train_df = pd.read_csv("train_day7.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
    
    #val_df = pd.read_csv("val_day9.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
 
    train_df = pd.read_csv("train_day8.csv", parse_dates=['click_time'], nrows=52000000, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
    
    val_df = pd.read_csv("val_day9.csv", parse_dates=['click_time'], nrows=12500000, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
    ...
    for i in range(0,naddfeat):
        ...
        if i==9: selcols=['ip','day','channel','hour']; QQ=1;#1
        if i==10: selcols=['ip','app','channel','day']; QQ=0;#1
        if i==11: selcols=['ip', 'day','app', 'device', 'os', 'channel','hour']; QQ=0;#1
        if i==12: selcols=['ip', 'day','device', 'channel','hour']; QQ=0;#1
        # if i==12: selcols=['ip', 'day','device', 'os', 'channel','hour']; QQ=0;#1
        # if i==10: selcols=['ip','day','app','channel','hour']; QQ=0;#1
    ...
    #train_df = train_df.loc[train_df.day <= 2]
    #val_df = train_df.loc[train_df.day == 3]
