...
print('load train....')
# we save only day 9
train_df = pd.read_csv(path+"train.csv", dtype=dtypes, skiprows = range(1, 181886954), usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'], parse_dates=['click_time'])
...
print('grouping by ip_device_os_app combination....')
gp = train_df[['ip', 'device', 'os', 'app']].groupby(by=['ip', 'device', 'os'])[['app']].count().reset_index().rename(index=str, columns={'app': 'ip_device_os_app'})
train_df = train_df.merge(gp, on=['ip','device', 'os', 'app'], how='left')
del gp; gc.collect()
...
train_df['ip_device_os_app'] = train_df['ip_device_os_app'].astype('uint16')
...
max_c3 = np.max([train_df['ip_device_os_app'].max(), test_df['ip_device_os_app'].max()])+1
...
def get_keras_data(dataset):
    X = {
        ...
        'c3': np.array(dataset.ip_device_os_app), 
...
emb_c3 = Embedding(max_c3, emb_n)(in_c2) 
in_c3 = Input(shape=[1], name = 'c3')

emb_c2 = Embedding(max_c2, emb_n)(in_c3) 
in_nc = Input(shape=[1], name = 'nc')


fe = concatenate([(emb_app), (emb_ch), (emb_dev), (emb_os), (emb_h), 
                 (emb_d), (emb_wd), (emb_qty), (emb_c1), (emb_c3),(emb_c2)])
...
model = Model(inputs=[in_app,in_ch,in_dev,in_os,in_h,in_d,in_wd,in_qty,in_c1,in_c2,in_c3,in_nc], outputs=outp)
...
del test_df; gc.collect()
print("writing....")
sub.to_csv('day789_feat12.csv',index=False)
